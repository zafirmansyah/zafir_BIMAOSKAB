...
class Mstsurat_template extends Bismillah_Controller
{   

    public function __construct(){
        parent::__construct() ;
        $this->load->model('mst/mstsurat_template_m') ;
        $this->load->helper('bdate') ;

        $this->bdb = $this->mstsurat_template_m ;
    }

    public function index(){
        $this->load->view('mst/mstsurat_template') ;
    }
    ...
    public function loadgrid(){
        ...
        while( $dbr = $this->bdb->getrow($dbd) ){
            $vaset                  = $dbr ;
            $cURLFile               = $this->bdb->getval("FilePath","Kode = '{$dbr['Kode']}'","template_dokumen_file") ;
            $vaset['cmdedit']       = "";
            $vaset['cmddelete']     = "";
            
            if(getsession($this,"Jabatan") == "002"){
                $vaset['cmdedit']       = '<button type="button" onClick="bos.mstsurat_template.cmdedit(\''.$dbr['Kode'].'\')"
                                        class="btn btn-success btn-grid">Edit</button>' ;
                $vaset['cmddelete']     = '<button type="button" onClick="bos.mstsurat_template.cmddelete(\''.$dbr['Kode'].'\')"
                                            class="btn btn-danger btn-grid">Delete</button>' ;
            }
            $vaset['cmdDownload']   = '<button type="button" onClick="bos.mstsurat_template.cmdDownload(\''.$cURLFile.'\')"
    ...
    public function saving(){
        ...
        $upload         = array("cUplFileTEMPLATE"=>getsession($this, "ssmstsurat_template_cUplFileTEMPLATE")) ;
        $va['FilePath'] = ""; 
        $dir            = "" ;
        if(!empty($upload)){
    ...
    public function savingFile()
    {
        ...
        $fcfg   = array("upload_path"=>"./tmp/","allowed_types"=>"pdf|doc|docx|xls|xlsx","overwrite"=>true) ;
            ...
            for($i = 0; $i < $nTotalFile; $i++){
                ...
                if ( ! $this->upload->do_upload("file") ){
                    echo('
                        ...
                        bos.mstsurat_template.obj.find("#idcUplFileTEMPLATE").html("") ;
                    ') ;
                }else{
                    ...
                    savesession($this, "ssmstsurat_template_cUplFileTEMPLATE", $vFile ) ;
                    echo('
                        //bos.mstsurat_template.obj.find("#idcUplFileTEMPLATE").html("") ;
                        //bos.mstsurat_template.obj.find("#idcUplFileTEMPLATE").html("<p>Data Uploaded<p>") ;
                    ') ;
                }
            }
        }
